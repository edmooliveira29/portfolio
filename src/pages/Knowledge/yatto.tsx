...
const DetailsYatto = ({ setShowDetails, showDetails }: Props) => {
  ...
  return (
    <Dialog open={open} onClose={() => { setOpen(false); setShowDetails({ ...showDetails, yatto: false }) }} className="relative z-10 lg:w-[80%]">
      <DialogBackdrop
        transition
            ...
            <div className="bg-gray-900">
              ...
              <div className="lg:flex text-white text-sm">
                <h1 className="text-2xl font-bold flex lg:hidden justify-between p-4">
                  ...
                  <CgClose size={25} onClick={() => { setOpen(false); setShowDetails({ ...showDetails, yatto: false }); }} />
                </h1>
                <div className="flex w-full lg:w-1/2 items-center">
                  <div className="m-5">
                    <Image src={yatto.src} alt="Logo Nolasco Moddas" width={yatto.width} height={yatto.height} className='h-auto' />
                  </div>
                </div>
                <div className="w-full lg:w-1/2 p-5">
                  <h1 className="text-3xl font-bold hidden lg:flex justify-between">
                    DSS TECHNOLOGY - YATTO
                    <CgClose size={25} onClick={() => { setOpen(false); setShowDetails({ ...showDetails, yatto: false }); }} className='cursor-pointer' />
                  </h1>
                  ...
                  <p>Atualmente atuo neste projeto como freelancer a <strong>{experienceTime('2024-05-01')}</strong>, sendo uma excelente oportunidade para aplicar e aprimorar minhas
